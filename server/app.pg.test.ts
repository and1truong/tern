...
test.skipIf(!url)('migration dry-run approval is bound to the selected schema', async () => {
  ...
  await post('access', { ...source, writable: true });
  try {
    expect((await post('datasource/exec', { ...source, sql: 'CREATE SCHEMA tern_gate_a; CREATE SCHEMA tern_gate_b', allowWrite: true })).ok).toBe(true);
    const migration = { ...source, sql: 'CREATE TABLE gated(id int)', schema: 'tern_gate_a', allowWrite: true };
    expect((await post('datasource/migration/preview', migration)).ok).toBe(true);
    const wrong = await post('datasource/migration/apply', { ...migration, schema: 'tern_gate_b' });
    expect(wrong.status).toBe(400);
    expect((await wrong.json()).error).toContain('dry run');
    expect((await post('datasource/migration/apply', migration)).ok).toBe(true);
    const query = await post('datasource/query', { ...source, schema: 'tern_gate_b', sql: "SELECT to_regclass('gated') AS name" });
    expect((await query.json()).rows[0].name).toBeNull();
  } finally {
    await post('datasource/exec', { ...source, sql: 'DROP SCHEMA IF EXISTS tern_gate_a CASCADE; DROP SCHEMA IF EXISTS tern_gate_b CASCADE', allowWrite: true });
    db.close();
  }
});
