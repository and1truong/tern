...
import { applyPgSchema, runPgQuery, runPgExec, explainPgQuery, runPgMigration } from './run.ts';
...
live('schema discovery, concurrent execution, scripts, explain, pagination, export and migrations keep their schema', async () => {
  ...
  try {
    ...
    expect((await readPgSchema(url, undefined, true)).tables.some(t => t.schema === 'pg_catalog')).toBe(true);
    ...
    await runPgMigration(url, migration, false, undefined, undefined, b);
    expect((await query(b, "SELECT to_regclass('migrated')::text AS name")).rows[0].name).toBeNull();
    await runPgMigration(url, migration, true, undefined, undefined, b);
