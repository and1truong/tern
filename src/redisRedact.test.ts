...
describe("redactSensitive", () => {
  test("redacts AUTH passwords, partial while typing", () => {
    expect(redactSensitive("AUTH s3cret")).toBe("AUTH (redacted)");
    expect(redactSensitive("AUTH default s3cret")).toBe("AUTH default (redacted)");
    expect(redactSensitive("AUTH sec")).toBe("AUTH (redacted)");
    expect(redactSensitive("AUTH")).toBe("AUTH");
  });
  test("redacts HELLO AUTH username and password", () => {
    expect(redactSensitive("HELLO 3 AUTH default s3cret")).toBe("HELLO 3 AUTH (redacted) (redacted)");
    expect(redactSensitive("HELLO 3 AUTH default")).toBe("HELLO 3 AUTH (redacted)");
