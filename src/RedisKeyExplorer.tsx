        ...
        <div key={key} className="group">
          {renaming?.from === key ? <div className="px-3 py-1 text-xs space-y-1">
            ...
            <input aria-label="Seconds until expiry" autoFocus type="number" min="1" value={expiring.seconds} onChange={e => setExpiring({ ...expiring, seconds: e.target.value })}
              onKeyDown={e => { if (e.key === 'Enter') void op({ op: 'expire', key, seconds: Number(expiring.seconds) }); }} className="w-full bg-[var(--bg)] border border-[var(--border)] p-1" />
            <div className="flex gap-1"><button className="primary text-xs" disabled={busy} onClick={() => void op({ op: 'expire', key, seconds: Number(expiring.seconds) })}>Expire</button><button className="text-xs" onClick={() => setExpiring(null)}>Cancel</button></div>
